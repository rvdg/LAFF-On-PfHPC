...
void GemmJI_Kernel_MRxNR( int, int, int, double *, int, double *, int, double *, int );
  ...
  for ( size=first; size<= last; size+=inc ){
    ...
    */
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmJI_Kernel_MRxNR( m, n, k, Ap, lda, Bp, ldb, Cp, ldc );
    ...
    printf( "data_GemmJI_Kernel_MRxNR( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
