#include <mpi.h>
int MST_Gatherx_aux( int *, int *, int, MPI_Comm, int, int );

int MST_Gatherx( int *buf, int *displs, int root, MPI_Comm comm)
{
  int nprocs;

  MPI_Comm_size( MPI_COMM_WORLD, &nprocs );
  MST_Gatherx_aux( buf, displs, root, comm, 0, nprocs-1 );
}

int MST_Gatherx_aux( int *buf, int *displs, 
		      int root, MPI_Comm comm, int left, int right )
{
  ...
  if ( left != right ){
	...
	MPI_Recv( &buf[ displs[ mid+1 ] ], displs[ right+1 ] - displs[ mid+1 ],
                  MPI_INT, new_root, scat_tag, comm, &status );
      else
