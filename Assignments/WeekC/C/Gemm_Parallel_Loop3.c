...
#pragma omp parallel
  {
    ...
    if ( tid == 0 ){
      ...
      Atilde = ( double * ) malloc( MC * KC * sizeof( double ) * num_threads );
    }
  }
...
#pragma omp parallel for
  for ( int i=0; i<m; i+=MC ) {
    ...
    PackBlockA_MCxKC( ib, k, &alpha( i, 0 ), ldA, Atilde+tid*MC*KC );
    LoopTwo( ib, n, k, Atilde+tid*MC*KC, Btilde, &gamma( i,0 ), ldC );
  }
}
