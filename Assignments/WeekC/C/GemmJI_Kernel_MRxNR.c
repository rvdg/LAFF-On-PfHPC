...
void GemmJI_Kernel_MRxNR( int, int, int, double *, int,
...
{
  if ( m % MR != 0 ){
    printf( "m must be multiple of MR\n" );
    exit( 0 );
  }
  if ( n % NR != 0 ){
    printf( "n must be multiple of NR\n" );
    exit( 0 );
  }

  GemmJI_Kernel_MRxNR( m, n, k, A, ldA, B, ldB, C, ldC );
}
