...
void GemmIJSapdot4x1Reg( int, int, int, double *, int, double *, int, double *, int );
...
int main(int argc, char *argv[])
{
  ...
  for ( size=first; size<= last; size+=inc ){
    ...
    /* Allocate space for the matrices.  We will use five arrays:
       ...
       Now, we will compute C = A B + C with the routine in GemmIJSapdot4x1Reg
    ...
    */
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmIJSapdot4x1Reg( m, n, k, ap, lda, bp, ldb, cp, ldc );
    ...
    printf( "data_GemmIJSapdot4x1Reg( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
