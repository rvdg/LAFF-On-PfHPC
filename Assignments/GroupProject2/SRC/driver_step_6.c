...
void GemmCacheBlocking( int, int, int, double *, int, double *, int, double *, int );
  ...
  for ( size=first; size<= last; size+=inc ){
    ...
    */
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmCacheBlocking( m, n, k, ap, lda, bp, ldb, cp, ldc );
    ...
    printf( "data_GemmCacheBlocking( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
