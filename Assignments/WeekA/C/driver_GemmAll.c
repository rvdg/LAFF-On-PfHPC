...
void GemmJIP( int, int, int, double *, int, double *, int, double *, int );

void GemmIPJ( int, int, int, double *, int, double *, int, double *, int );
void GemmPIJ( int, int, int, double *, int, double *, int, double *, int );

void GemmPJI( int, int, int, double *, int, double *, int, double *, int );
void GemmJPI( int, int, int, double *, int, double *, int, double *, int );
  ...
  for ( size=first; size<= last; size+=inc ){
    ...
    */
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmJIP( m, n, k, Ap, lda, Bp, ldb, Cp, ldc );
    ...
    printf( "data_GemmJIP( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmIPJ( m, n, k, Ap, lda, Bp, ldb, Cp, ldc );
    ...
    printf( "data_GemmIPJ( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmPIJ( m, n, k, Ap, lda, Bp, ldb, Cp, ldc );
    ...
    printf( "data_GemmPIJ( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmJPI( m, n, k, Ap, lda, Bp, ldb, Cp, ldc );
    ...
    printf( "data_GemmJPI( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      /* Compute C = A B + C */
      GemmPJI( m, n, k, Ap, lda, Bp, ldb, Cp, ldc );
    ...
    printf( "data_GemmPJI( %d, 1:3 ) = [ %d %le %le];\n",
	    i, n, dtime_best, diff  );
    fflush( stdout );  // We flush the output buffer because otherwise
		       // it may throw the timings of a next
		       // experiment.
